...
import { 
    collection, query, where, onSnapshot, updateDoc, doc, arrayUnion 
...
export function initNotificationFeature() {
    ...
    // FIX: Remove 'orderBy' to prevent "Missing Index" errors on client side.
    ...
    onSnapshot(qGlobal, (snapshot) => {
        ...
        if(snapshot.empty) {
             list.innerHTML = `<li style="padding:15px; text-align:center; color:#888;">No notifications</li>`;
        ...
        list.innerHTML = `<li style="padding:15px; text-align:center; color:red;">Notifications unavailable.</li>`;
    });
